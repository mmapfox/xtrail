...
void mouseUpdate(Mouse *mouse) {
    ...
    if (mouse->loopIndex % 10 == 0) { // don't check too frequently
        ...
        if (cursor_image) {
            ...
            if (wasHidden && !mouse->hidden) {
                mouse->forbidRenderTicks = 10;
            }
            if (mouse->hidden) {
                mouse->listc = 0;
    ...
    if (mouse->forbidRenderTicks > 0) {
        mouse->listc = 0;
        mouse->forbidRenderTicks--;
    }
    ...
    mouse->lastMove++;
    MouseState mold = mouseState(mouse, 1);
    if (mcurr.p.x != mold.p.x || mcurr.p.y != mold.p.y || 0x700 & mcurr.mask) {
        mouse->lastMove = 0;
    }
}
...
void mouseReset(Mouse *mouse) {
    ...
    mouse->listc = 0;
    mouse->loopIndex = 0;
    mouse->lastMove = mouse->historyBufSize*20;
    mouse->forbidRenderTicks = 0;
    mouseUpdate(mouse);
}
